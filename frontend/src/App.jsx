...
function AppHeader({ settings, syncStatus, activeTab, onSelectTab }){
  return (
    <header className="g5-header">
      <div className="g5-header-row">
        ...
        <div className="g5-header-right">
          ...
          <span className="g5-cycle-chip"><span className="g5-cycle-dot" /><span className="g5-cycle-text">{settings.cycleLabel} · {settings.cyclePeriod}</span></span>
        </div>
      </div>
...
export default function App(){
  const [areas, persistAreas, statusAreas] = useCloudCollection("g5s:areas", SEED_AREAS);
  ...
  const [settings, persistSettings, statusSettings] = useCloudCollection("g5s:settings", SEED_SETTINGS);
  ...
  function importAll(parsed){
    if (Array.isArray(parsed.areas)) persistAreas(() => parsed.areas);
    ...
    if (parsed.settings && typeof parsed.settings === "object") persistSettings(() => parsed.settings);
    setToast("Backup importado com sucesso");
  }
  function resetAll(){
    persistAreas(() => SEED_AREAS);
    persistMasterPlan(() => SEED_MASTERPLAN);
    persistCommittee(() => SEED_COMMITTEE);
    persistCronograma(() => SEED_CRONOGRAMA);
    persistSettings(() => SEED_SETTINGS);
    setSelectedAreaId(null);
    setToast("Dados originais restaurados");
  }
