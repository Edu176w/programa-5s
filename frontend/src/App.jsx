...
/* ================================================================
   ...
   ================================================================ */
/* ----------------------------------------------------------------
   Moldes VAZIOS — usados como ponto de partida pra qualquer empresa
   nova cadastrada na Plataforma (não a Usina Serra Grande, cujos
   dados reais já existem gravados no banco desde a migração inicial
...
function PlatformView({ ownCompanyId, currentUserId, onEnterCompany, onEnterSupport }){
  ...
  async function confirmDeleteCompany(){
    ...
    try {
      const res = await apiFetch(`/api/platform/companies/${deleting.id}`, { method: "DELETE" });
      if (!res.ok){
        const json = await res.json().catch(()=>({}));
        setDeleteError(json.error === "cannot_delete_own_company"
          ? "Não é possível excluir sua própria empresa por aqui."
          : "Não foi possível excluir a empresa.");
        setDeletingBusy(false);
        return;
      }
      setDeleting(null); setDeleteConfirmText(""); setDeletingBusy(false);
      await loadCompanies();
    } catch {
      setDeleteError("Não foi possível excluir a empresa.");
      setDeletingBusy(false);
    }
  }
  ...
  return (
    <div>
      ...
      {deleting && (
        <Modal title="Excluir empresa" onClose={()=>setDeleting(null)}
          footer={
            <>
              <button className="g5-btn g5-btn-outline" onClick={()=>setDeleting(null)}>Cancelar</button>
              <button className="g5-btn g5-btn-danger" disabled={deleteConfirmText.trim() !== deleting.name || deletingBusy}
                onClick={confirmDeleteCompany}>{deletingBusy ? "Excluindo…" : "Excluir definitivamente"}</button>
            </>
          }>
          <p style={{ marginBottom:12 }}>
            Isso apaga <b>{deleting.name}</b> para sempre — todas as áreas, PAD, comitê, cronograma, plano geral e as contas de usuário dela.
            Não pode ser desfeito.
          </p>
          <div className="g5-field">
            <label className="g5-label">Digite "{deleting.name}" para confirmar</label>
            <input className="g5-input" value={deleteConfirmText} onChange={e=>setDeleteConfirmText(e.target.value)} autoFocus />
          </div>
          {deleteError && <p className="g5-login-error">{deleteError}</p>}
        </Modal>
      )}
    </div>
  );
...
function App({ session, onUpdateCompany, onLogout }){
  ...
  const [areasByCycle, persistAreasByCycle, statusAreas] = useCloudCollection("g5s:areas", EMPTY_AREAS_BY_CYCLE);
  const [masterPlan, persistMasterPlan, statusMP] = useCloudCollection("g5s:masterplan", EMPTY_MASTERPLAN);
  const [committee, persistCommittee, statusCommittee] = useCloudCollection("g5s:committee", EMPTY_COMMITTEE);
  ...
  function selectTab(tabId){
    if (tabId === "platform"){
      if (session.impersonating){
        // Em modo suporte, "Plataforma" sai do modo suporte de vez (troca de
        // volta pro token/empresa original do dono) em vez de só trocar de
        // aba — continuar "dentro" do token da empresa alheia não faria
        // sentido depois de voltar pra tela de todas as empresas.
        exitSupportMode();
        return;
      }
      // Voltar pra "Plataforma" sempre volta ao estado de landing (esconde
      // as abas operacionais de novo), mesmo vindo de dentro da própria empresa.
  ...
  function resetAll(){
    persistAreasByCycle(() => EMPTY_AREAS_BY_CYCLE);
    persistMasterPlan(() => EMPTY_MASTERPLAN);
    persistCommittee(() => EMPTY_COMMITTEE);
    persistCronograma(() => EMPTY_CRONOGRAMA);
    persistSettingsRaw(() => EMPTY_SETTINGS);
    setSelectedAreaId(null);
    setToast("Todos os dados desta empresa foram apagados");
  }
